...
class ParticleFilter:
    ...
    def save(self, filename: str):
        with open(filename, mode='w') as csv_file:
            ...
            fieldnames= ["x_position", "y_position", "row_angle"]

            for i in range(10):
     
                fieldnames.append(f"x_position{i}")
                fieldnames.append(f"y_position{i}")
                fieldnames.append(f"row_angle{i}")
    
    
            writer = csv.DictWriter(csv_file, fieldnames=fieldnames)

            data = {'x_position': "1.0", 'y_position': "2.0", 'row_angle': "3.0"}
    
            for i in range(10):
    
                data[f"x_position{i}"] =  self.Particle.position[0]
                data[f"y_position{i}"] =  self.Particle.position[1]
                data[f"row_angle{i}"] =  self.Particle.yaw
